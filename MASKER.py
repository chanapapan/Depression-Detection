...
if debug == True:
    save_dir      = f'./save/debug-{training_obj}-{keyword_name}-r{class_ratio}'
    writer        = SummaryWriter(log_dir = f'./tensorboard_runs/FINALv1_runs/debug-{training_obj}-{keyword_name}-r{class_ratio}')
    num_epoch     = 1
    val_steps     = 10
    logging_steps = 10
else:
    save_dir      = f'./save/{training_obj}-{keyword_name}-r{class_ratio}'
    writer        = SummaryWriter(log_dir = f'./tensorboard_runs/FINALv1_runs/{training_obj}-{keyword_name}-r{class_ratio}')
...
class MaskerNet(nn.Module):
    ...
    def forward(self, X_s):
        ...
        x_orig, x_mask, x_ood =  X_s[0], X_s[1], X_s[2]
...
if training_obj == 'MASKER' :
    ...
    model     = MaskerNet(backbone, tokenizer).to(device)
...
class CustomTrainer():
    def __init__(self, training_arg, TrainEval):
        ...
        if training_obj == 'MASKER':
            ...
            self.train_one_step = TrainEval.train_one_step_MASKER
