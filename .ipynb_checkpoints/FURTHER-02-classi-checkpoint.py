...
load_model_from = './save/FURTHER-01-MLM-NNrandom/best-model-60000.tar'
...
if debug == True:
    save_dir      = f'./save/debug-FURTHER-02-{training_obj}-{keyword_name}-r{class_ratio}'
    writer        = SummaryWriter(log_dir = f'./tensorboard_runs/FINALv1_runs/debug-FURTHER-02-{training_obj}-{keyword_name}-r{class_ratio}')
    num_epoch     = 1
    val_steps     = 10
    logging_steps = 10
else:
    save_dir      = f'./save/FURTHER-02-{training_obj}-{keyword_name}-r{class_ratio}'
    writer        = SummaryWriter(log_dir = f'./tensorboard_runs/FINALv1_runs/FURTHER-02-{training_obj}-{keyword_name}-r{class_ratio}')
...
print(f"Loading dataset from ./data/classi/classichunk-R{class_ratio}-train-ds.pkl")
...
training_arg = {
                ...
                'load_model_from' : load_model_from, # saved model path
...
class CustomTrainer():
    ...
    def load_checkpoint(self,): # only for continuing the next task (NOT for continue training)
        ...
        print(self.model.load_state_dict(checkpoint['model_state_dict'], strict=False)) # <All keys matched successfully>
