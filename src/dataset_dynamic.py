...
#                         print(word_length_list)
        ...
        idx_mask_all     = torch.tensor([mask_idx for word_id in word_ids_to_mask for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        idx_maskmask     = torch.tensor([mask_idx for word_id in word_ids_maskmask for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        idx_maskrandom   = torch.tensor([mask_idx for word_id in word_ids_maskrandom for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        idx_maskoriginal = torch.tensor([mask_idx for word_id in word_ids_maskoriginal for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        ...
        # random tokens 10%
        input_ids.index_put_(indices = (idx_maskrandom, ) , values = torch.tensor(random.sample(self.vocab, len(idx_maskrandom))))
        ...
        sample = {  'input_ids'      : input_ids.clone(),
                    'word_ids'       : word_ids.clone(),
                    'attention_mask' : torch.ones_like(input_ids),
                    'orig_text'      : orig_text,
                    'masked_text'    : self.tokenizer.decode(input_ids.clone()),
                    'labels'         : label.clone()}
        return sample
...
class MASKERDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        idx_key_mask     = torch.tensor([mask_idx for word_id in word_ids_to_mask for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        idx_out_mask     = torch.tensor([mask_idx for word_id in word_ids_out_mask for mask_idx in range(word_ids.shape[0]) if word_id == word_ids[mask_idx]])
        ...
        MKR_labels.index_put_(indices = (idx_key_mask, ) , values = torch.index_select(orig_input_ids, dim = 0 , index= torch.tensor(idx_key_mask)))
        ...
        sample = {  'all_input_ids'  : (orig_token, MKR_tokens, MER_tokens),
                    'word_ids'       : word_ids.clone(),
                    'orig_text'      : orig_text,
                    'all_labels'     : (MKR_labels, labels)}
        return sample
...
# class PROPMaskedDomainDataset(Dataset):
    
#     def __init__(self, data_loader, classitoken_model, model_mask_id):

#         self.mask_input_ids_label(data_loader, classitoken_model, model_mask_id)
...
#     def mask_input_ids_label(self, data_loader, model, model_mask_id):
        
#         all_input_ids = []
#         all_labels    = []
#         # all_orig_text       = []
        
#         self.count_masked_tokens = []
#         self.masked_input_ids = []
...
#             pred = torch.argmax(torch.softmax(outputs.logits.detach(), dim = 2), dim = 2)
#             # print(pred)
...
#             labels    = torch.ones_like(att_mask).to(device) * -100
...
#             self.count_masked_tokens.append(torch.sum(torch.isin(masked_labels, input_ids)).clone().detach().cpu().tolist())
#             self.masked_input_ids.append(important_input_ids.clone().detach().cpu().tolist())
            
#             # print(input_ids[0])
#             # print(masked_input_ids[0])
#             # print(masked_labels[0])

#             all_input_ids.append(masked_input_ids.detach().cpu())
#             all_labels.append(masked_labels.detach().cpu())
#             # all_orig_text.append(batch['orig_text'])
            
#             # break
        
#         self.all_input_ids  = torch.cat(all_input_ids, dim = 0).cpu()
#         self.all_labels     = torch.cat(all_labels, dim = 0).cou()
#         # self.all_orig_text  = all_orig_text
        
# class NNMaskedDomainDataset(Dataset):
...
#             # index of important tokens in each sequence
#             important_idx_seq = (pred == 1).nonzero(as_tuple=True)[0]
#             important_idx_pos = (pred == 1).nonzero(as_tuple=True)[1]
#             # print(important_idx_seq, important_idx_pos)
...
#             all_input_ids.append(masked_input_ids.detach())
#             all_labels.append(masked_labels.detach())
#             # all_orig_text.append(data['orig_text'])
        
#         self.all_input_ids = torch.cat(all_input_ids, dim = 0)
